...
export const kanban = async (req: Request, res: Response): Promise<Response> => {
  ...
  const { tickets, count, hasMore } = await ListTicketsServiceKanban({
    ...
    withUnreadMessages,
    companyId,
    funilId: funilId ? Number(funilId) : undefined
  });
...
export const countTickets = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const count = await Ticket.count({ where: { companyId } });
    return res.status(200).json({ count });
  } catch (error) {
    console.error("Erro ao contar tickets:", error);
    return res.status(500).json({ message: "Erro ao contar tickets." });
  }
};