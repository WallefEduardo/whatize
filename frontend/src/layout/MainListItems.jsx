...
// Estilos convertidos para usar com sx prop
const styles = {
  listItem: {
    height: "48px",
    width: "auto",
      ...
      <li>
        <ListItem button component={renderLink} sx={styles.listItem}>
          {icon ? (
            <ListItemIcon>
              {showBadge ? (
                <Badge badgeContent="!" color="error" overlap="circular" sx={styles.badge}>
                  <Avatar sx={{...styles.iconHoverActive, ...(isActive && {"&.active": styles.iconHoverActive["&:hover, &.active"]})}}>{icon}</Avatar>
                </Badge>
              ) : (
                <Avatar sx={{...styles.iconHoverActive, ...(isActive && {"&.active": styles.iconHoverActive["&:hover, &.active"]})}}>{icon}</Avatar>
              )}
...
const MainListItems = ({ collapsed, drawerClose }) => {
  ...
  const location = useLocation();
  const [drawerOpen, setDrawerOpen] = useState(collapsed !== true);
  ...
  // Verificar se estamos na página de financeiro-aberto
  const isFinanceiroAberto = location.pathname === "/financeiro-aberto" || 
                            (location.pathname === "/financeiro" && window.location.href.includes("/financeiro-aberto")) || 
  ...
  // Verificar se a assinatura está realmente vencida com base no dueDate
  useEffect(() => {
    if (user && user.company && user.company.dueDate) {
      const dueDate = user.company.dueDate;
      const before = moment(moment().format()).isBefore(dueDate);
      
      if (before) {
        // Assinatura está válida
        localStorage.removeItem("assinaturaVencida");
      } else {
        // Assinatura está vencida
  ...
  useEffect(() => {
    if (user.id) {
      ...
      const onCompanyChatMainListItems = async (data) => {
        if (data.action === "new-message") {
          try {
            const countResponse = await api.get("/tickets/count");
            const { count } = countResponse.data;

            if (count >= 50) {
              console.error("Limite de 50 tickets atingido. Atualize seu plano para criar mais tickets.");
              return; // Interrompe a criação do ticket
            }

            dispatch({ type: "CHANGE_CHAT", payload: data })
          } catch (error) {
            console.error("Erro ao verificar ou criar o ticket:", error);
          }
          dispatch({ type: "CHANGE_CHAT", payload: data });
        }
  ...
  if (isFinanceiroAberto) {
    return (
      <div onClick={drawerClose}>
        <ListItemLink
          to="/financeiro-aberto"
    ...
    <div onClick={drawerClose}>
          ...
          <>
            <Tooltip title={collapsed ? i18n.t("mainDrawer.listItems.management") : ""} placement="right">
              ...
              >
                <ListItemIcon>
                  <Avatar
                    sx={{...styles.iconHoverActive, ...(isManagementActive || managementHover && styles.iconHoverActive["&:hover, &.active"])}}
                  >
                    <HomeTrendUp />
                  </Avatar>
                </ListItemIcon>
            ...
            <Collapse
              ...
              unmountOnExit
              style={{
                backgroundColor: "light" === "light" ? "rgba(120,120,120,0.1)" : "rgba(120,120,120,0.5)",
              }}
            >
              <Can
      ...
      <Can
        ...
        yes={() => (
          <>
            {!collapsed && (
              <ListSubheader sx={styles.listSubheader} inset>{i18n.t("mainDrawer.listItems.administration")}</ListSubheader>
            )}
            {showCampaigns && (
              <Can
                role={user.profile}
                  ...
                  <>
                    <Tooltip title={collapsed ? i18n.t("mainDrawer.listItems.campaigns") : ""} placement="right">
                      ...
                      >
                        <ListItemIcon>
                          <Avatar
                            sx={{...styles.iconHoverActive, ...(isCampaignRouteActive || campaignHover && styles.iconHoverActive["&:hover, &.active"])}}
                          >
                            <VolumeHigh />
                          </Avatar>
                        </ListItemIcon>
                ...
                <>
                  <Tooltip title={collapsed ? i18n.t("mainDrawer.listItems.campaigns") : ""} placement="right">
                    ...
                    >
                      <ListItemIcon>
                        <Avatar
                          sx={{...styles.iconHoverActive, ...(isFlowbuilderRouteActive || flowHover && styles.iconHoverActive["&:hover, &.active"])}}
                        >
                          <HierarchySquare3 />
                        </Avatar>
                      </ListItemIcon>
