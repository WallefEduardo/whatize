...
// Animações recriadas com sintaxe correta
...
// Componentes estilizados recriados
...
const StepItem = styled(Box, {
  ...
  border: completed 
    ? '1px solid rgba(0, 195, 7, 0.3)' 
    : active 
      ? '1px solid rgba(255, 255, 255, 0.2)' 
      : '1px solid rgba(255, 255, 255, 0.1)',
  transition: 'all 0.3s ease',
  animation: active ? `${slideUpAnimation} 0.5s ease-out` : 'none',
  '& .step-icon': {
    width: '24px',
    height: '24px',
    borderRadius: '50%',
    ...
    '& .MuiSvgIcon-root': {
      ...
      animation: (active && !completed) ? `${rotateAnimation} 2s linear infinite` : 'none',
    },
  },
  '& .step-text': {
    color: completed 
...
const ProgressModal = (props) => {
  const { open, progress = 0, currentStep = 0, message = "Preparando sua conta..." } = props;
  const steps = [
