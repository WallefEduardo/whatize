...
// MicRecorder será inicializado dentro do componente
...
const MessageInput = ({ ticketId, ticketStatus, droppedFiles, contactId, ticketChannel, ticket }) => {
  ...
  // Styles converted from makeStyles to inline styles
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (err.response && err.response.status === 403) {
        toastError("Limite de 50 chamados atingido. Para enviar novas mensagens, entre em contato com o suporte.");
      } else {
        toastError("Ocorreu um erro ao enviar a mensagem.");
  ...
  const renderReplyingMessage = (message) => {
    return (
      <div style={styles.replyginMsgWrapper}>
        <div style={styles.replyginMsgContainer}>
          <span
            style={!message.fromMe ? styles.replyginSelfMsgSideColor : styles.replyginContactMsgSideColor}
          ></span>
          {replyingMessage && (
            <div style={styles.replyginMsgBody}>
              {!message.fromMe && (
                <span style={styles.messageContactName}>
        ...
        >
          <Clear style={styles.sendMessageIcons} />
        </IconButton>
      </div>
    );
  };
  ...
  useEffect(() => {
    const handleTyping = () => {
      ...
      socket.emit('user_typing', { ticketId });
    };
  ...
  if (mediasUpload.length > 0) {
    return (
      ...
      <Paper
        elevation={0}
        square
        style={styles.viewMediaInputWrapper}
    ...
    return (
      <>
        ...
        >
          ...
          <div style={styles.newMessageBox}>
            {isMdUp && (
              <>
                <IconButton
                ...
                >
                  <Mood style={styles.sendMessageIcons} />
                </IconButton>
                {showEmoji && (
              ...
              >
                <MenuItem onClick={handleMenuItemClick}>
                  ...
                  <label htmlFor="upload-img-button">
                    <Fab
                      ...
                      component="span"
                      style={styles.invertedFabMenuMP}
                    >
                      <PermMedia />
                    </Fab>
                ...
                <MenuItem onClick={handleCameraModalOpen}>
                  <Fab style={styles.invertedFabMenuCamera}>
                    <CameraAlt />
                  </Fab>
                ...
                <MenuItem onClick={handleMenuItemClick}>
                  ...
                  <label htmlFor="upload-doc-button">
                    <Fab aria-label="upload-img"
                      component="span" style={styles.invertedFabMenuDoc}>
                      <Description />
                    </Fab>
                    Documento
                  </label>
                ...
                <MenuItem onClick={handleSendContactModalOpen}>
                  <Fab style={styles.invertedFabMenuCont}>
                    <Person />
                  </Fab>
                  {i18n.t("messageInput.type.contact")}
                </MenuItem>
                <MenuItem onClick={handleSendLinkVideo}>
                  <Fab style={styles.invertedFabMenuMeet}>
                    <Duo />
                  </Fab>
              ...
              {/* <IconButton
				...
				>
				  <AttachFile style={styles.sendMessageIcons} />
				</IconButton> */}
            ...
            {isSmDown && (
              <>
                ...
                <Tooltip title={i18n.t("tickets.buttons.quickmessageflash")}>
                  ...
                  >
                    <BoltIcon style={styles.sendMessageIcons} />
                  </IconButton>
                </Tooltip>
                <IconButton
                  aria-label="emojiPicker"
            ...
            <div style={styles.flexContainer}>
              {privateMessageInputVisible && (
                <div style={styles.flexItem}>
                  <div style={styles.messageInputWrapperPrivate}>
                    <InputBase
                      inputRef={(input) => {
                        input && input.focus();
                        input && (inputRef.current = input);
                      }}
                      style={styles.messageInputPrivate}
                      placeholder={
                        ticketStatus === "open" || ticketStatus === "group"
                    ...
                    />
                    {typeBar ? (
                      <ul style={styles.messageQuickAnswersWrapper}>
                        {typeBar.map((value, index) => {
                          return (
                            <li
                              style={styles.messageQuickAnswersWrapperItem}
                              key={index}
                            >
            ...
            {!privateMessageInputVisible && (
              <>
                <div style={{ display: window.innerWidth < 850 ? 'flex' : 'none' }}>
                  <IconButton
                    aria-label="emojiPicker"
                    component="span"
                    disabled={disableOption()}
                ...
                <Tooltip title={i18n.t("tickets.buttons.scredule")}>
                  <IconButton
                    ...
                    disabled={loading}
                  >
                    <Timer style={styles.sendMessageIcons} />
                  </IconButton>
                </Tooltip>

                <div style={{ marginRight: window.innerWidth < 850 && window.innerWidth > 598 ? '65px' : '0' }}>
                  ...
                  {inputMessage || showSelectMessageCheckbox ? (
                    ...
                    >
                      {showSelectMessageCheckbox ?
                        <Reply style={styles.ForwardMessageIcons} /> : <Send style={styles.sendMessageIcons} />}
                    </IconButton>
                  ) : recording ? (
                    <div style={{ ...styles.recorderWrapper, display: 'flex', justifyContent: 'space-between' }}>
                      <IconButton
                      ...
                      >
                        <HighlightOff style={styles.cancelAudioIcon} />
                      </IconButton>
                      {loading ? (
                        <div>
                          <CircularProgress style={styles.audioLoading} />
                        </div>
                      ) : (
                        <RecordingTimer />
                      )}
                      <IconButton
                        ...
                        disabled={loading}
                      >
                        <CheckCircleOutline style={styles.sendAudioIcon} />
                      </IconButton>
                    </div>
                  ) : (
                    <IconButton
                    ...
                    >
                      <Mic style={styles.sendMessageIcons} />
                    </IconButton>
                  )}
                </div>
