...
const FlowBuilderSingleBlockModal = ({
  ...
  const handleElements = newNameFiles => {
    ...
    for (let i = 0; i < numberPdf; i++) {
        ...
        const onlyPdfNameOriginal = medias.filter(file => file.name.includes("pdf"));
        
        if (elementsSeqEdit.includes(newArrPdf[i])) {
            const itemSelectedEdit = elementsEdit.filter(item => item.number === newArrPdf[i])[0];
            elementsSequence.push({
                type: "pdf",
                value: itemSelectedEdit.value,
                original: itemSelectedEdit.original,
            ...
            elementsSequence.push({
                type: "pdf",
                value: onlyPdf[i],
                original: onlyPdfNameOriginal[i].name,
                number: newArrPdf[i]
            });
        }
    }
  ...
  const handleChangePdfs = (e, number) => {
    if (!e.target.files) {
        return;
    }
    ...
    const pdfBlob = URL.createObjectURL(e.target.files[0]);
    setPreviewPdfs(old => [
        ...old,
        {
            number: number,
    ...
    document.querySelector(`.pdf${number}`).innerHTML = `<a href="${pdfBlob}" target="_blank" rel="noopener noreferrer">${e.target.files[0].name}</a>`;
    document.querySelector(`.btnPdf${number}`).remove();
  };
  ...
  const pdfLayout = (number, valueDefault = "") => {
    return (
        ...
        >
            <Stack sx={{ position: "absolute", right: 6 }}>
                <Delete onClick={() => deleteElementsTypeOne(number, "pdf")} />
            </Stack>
            <Typography textAlign={"center"}>PDF</Typography>
            <div className={`pdf${number}`}>
                {valueDefault.length > 0 && (
                    <a href={process.env.REACT_APP_BACKEND_URL + "/public/" + valueDefault} target="_blank" rel="noopener noreferrer">
                        {valueDefault}
                    </a>
                )}
            </div>
            {valueDefault.length === 0 && (
                ...
                >
                    ...
                    <input
                        ...
                        hidden
                        onChange={e => handleChangePdfs(e, number)}
                    />
                </Button>
            )}
        </Stack>
    ...
    if (data) {
      ...
      sequence.map(item => {
        ...
        if (itemNode.type === "pdf") {
          ...
          setElements(elm => [...elm, pdfLayout(numberLoc, itemNode.value)]);
          setNumberPdf(old => {
  ...
  const handleSaveNode = async () => {
    ...
    if (open === "edit") {
      ...
      medias.forEach(async (media, idx) => {
        ...
        console.log("media?.type open create", media?.type.split("/")[0])
        if (media?.type.split("/")[0] === "image") {
            new Compressor(file, {
              quality: 0.7,
